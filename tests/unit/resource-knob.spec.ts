...
describe('resourceKnob', () => {
  test('only Podman on a VM platform is CLI-applicable', () => {
    assert.equal(resourceKnob('memory', 'podman', 'win32').kind, 'cli');
    assert.equal(resourceKnob('memory', 'podman', 'darwin').kind, 'cli');
    assert.equal(resourceKnob('memory', 'docker', 'win32').kind, 'external');
    assert.equal(resourceKnob('memory', 'docker', 'darwin').kind, 'external');
  });

  test('native Linux has no knob for either engine', () => {
    for (const engine of ENGINES) {
      const knob = resourceKnob('memory', engine, 'linux');
  ...
  test('an omitted provider resolves to the platform default, not to unknown', () => {
    assert.equal(resourceKnob('memory', 'podman', 'win32').via, 'wslconfig');
    assert.equal(resourceKnob('memory', 'podman', 'darwin').via, 'machine-set');
  });

  test('an explicitly unknown provider stays unknown', () => {
    for (const platform of ['win32', 'darwin'] as NodeJS.Platform[]) {
      const knob = resourceKnob('memory', 'podman', platform, 'unknown');
  ...
  test('each Podman provider maps to the mechanism that actually works there', () => {
    ...
    assert.equal(resourceKnob('memory', 'podman', 'win32', 'wsl').via, 'wslconfig');
    for (const provider of ['hyperv', 'applehv', 'qemu'] as MachineProvider[]) {
      assert.equal(resourceKnob('memory', 'podman', 'win32', provider).via, 'machine-set', provider);
    }
  });

  test('the provider never changes the answer for Docker or for Linux', () => {
    for (const provider of PROVIDERS) {
      assert.equal(resourceKnob('memory', 'docker', 'win32', provider).kind, 'external', provider);
      assert.equal(resourceKnob('memory', 'docker', 'darwin', provider).kind, 'external', provider);
      for (const engine of ENGINES) {
        assert.equal(resourceKnob('memory', engine, 'linux', provider).kind, 'none', `${engine}/${provider}`);
      }
    }
  });
  ...
  test('Windows Docker follows the backend it was probed on', () => {
    const wsl = resourceKnob('memory', 'docker', 'win32', 'wsl');
    ...
    const hyperv = resourceKnob('memory', 'docker', 'win32', 'hyperv');
  ...
  test('an unprobed Windows Docker backend names both places, not one', () => {
    const knob = resourceKnob('memory', 'docker', 'win32');
  ...
  test('macOS Docker points at Docker Desktop settings', () => {
    assert.match(resourceKnob('memory', 'docker', 'darwin').where, /Docker Desktop/);
  });
  ...
  test('every non-cli answer carries a reason the apply planner can print', () => {
    for (const engine of ENGINES) {
      for (const platform of PLATFORMS) {
        for (const provider of PROVIDERS) {
          const knob = resourceKnob('memory', engine, platform, provider);
